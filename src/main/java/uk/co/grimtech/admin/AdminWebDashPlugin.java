...
public class AdminWebDashPlugin extends JavaPlugin {
  ...
  public static String getDiscordChannelLogs() {
    return discordChannelLogs;
  }

  public static String getDiscordChannelAlerts() {
    return discordChannelAlerts;
  }

  public static String getDiscordChannelJoins() {
    return discordChannelJoins;
  }

  public static String getDiscordCommandPrefix() {
  ...
  public static boolean isReverseProxy() {
    return reverseProxy;
  }

  public static boolean isLetsEncrypt() {
    return letsEncrypt;
  }

  public static String getLetsEncryptEmail() {
    return letsEncryptEmail;
  }

  public static int getLoginRateLimit() {
  ...
  public static java.util.List<String> getIpAllowlist() {
    return ipAllowlist;
  }
                          ...
                          if (MuteTracker.isMuted(playerUuid)) {
                            ...
                            if (mute != null) {
                              ...
                              if (mute.durationSeconds == null) {
                                ...
                                muteMsg =
                                    String.format(
                                        "You are muted for %dm %ds. Reason: %s",
                                        minutes, seconds, mute.reason);
                              }
                              event
                                  .getSender()
                                  .sendMessage(com.hypixel.hytale.server.core.Message.raw(muteMsg));
                            }
                            LOGGER.info(
                                "[AdminWebDash] Blocked chat from muted player: "
    ...
    try {
      if (configFile.exists()) {
        try (FileReader reader = new FileReader(configFile)) {
          ...
          if (config.has("port")) {
            ...
            if (configPort == 0) {
              ...
              if (port < 1024 || port > 65535) {
                LOGGER.warning(
                    "[AdminWebDash] Invalid port " + port + " in config, using default 9081");
                port = 9081;
              }
            }
          }
          if (config.has("discordEnabled"))
            discordEnabled = config.get("discordEnabled").getAsBoolean();
          if (config.has("discordToken")) discordToken = config.get("discordToken").getAsString();
          if (config.has("discordGuildId"))
            discordGuildId = config.get("discordGuildId").getAsString();
          if (config.has("discordChannelLogs"))
            discordChannelLogs = config.get("discordChannelLogs").getAsString();
          if (config.has("discordChannelAlerts"))
            discordChannelAlerts = config.get("discordChannelAlerts").getAsString();
          if (config.has("discordChannelJoins"))
            discordChannelJoins = config.get("discordChannelJoins").getAsString();
          if (config.has("discordCommandPrefix"))
            discordCommandPrefix = config.get("discordCommandPrefix").getAsString();
          ...
          if (config.has("keystorePassword"))
            keystorePassword = config.get("keystorePassword").getAsString();
          ...
          if (config.has("letsEncryptEmail"))
            letsEncryptEmail = config.get("letsEncryptEmail").getAsString();

          if (config.has("loginRateLimit"))
            loginRateLimit = config.get("loginRateLimit").getAsInt();
      ...
      if (LOGGER != null) {
        LOGGER.info(
            "[AdminWebDash] Config loaded - Port: "
                + (port == 0 ? "random" : port)
                + ", Logging enabled: "
                + loggingEnabled);
  ...
  public void updateBackupInterval(int interval) {
    ...
    try {
      ...
      LOGGER.info(
          "[AdminWebDash] Backup interval updated to " + interval + " and saved to config.");
    } catch (Exception e) {
  ...
  private void updateDiscordPresence() {
    ...
    try {
      int onlinePlayers =
          com.hypixel.hytale.server.core.universe.Universe.get().getPlayers().size();
  ...
  private class DiscordListener extends ListenerAdapter {
    ...
    public void onMessageReceived(@Nonnull MessageReceivedEvent event) {
      ...
      if (content.startsWith(discordCommandPrefix)
          && (discordChannelLogs.isEmpty() || channelId.equals(discordChannelLogs))) {
        ...
        try {
          ...
          if (args.length >= 2) {
            if (action.equals("heal")
                || action.equals("kick")
                || action.equals("ban")
                || action.equals("gamemode")
                || action.equals("give")
                || action.equals("mute")
                || action.equals("unmute")
                || action.equals("unban")
                || action.equals("clearinv")) {
          ...
          if (!isCustom) {
            ...
            CommandManager.get()
                .handleCommand((CommandSender) sender, cmd)
                .whenComplete(
                    (result, exception) -> {
                      if (exception != null) {
                        sender.sendMessage(
                            com.hypixel.hytale.server.core.Message.raw(
                                "[ERROR] Execution Exception: " + exception.getMessage()));
                      }
          ...
          event
              .getChannel()
              .sendMessage("[ERROR] Error dispatching command: " + e.getMessage())
              .queue();
        }
      }
    }
    ...
    private void handleCustomCommand(
      ...
      if (action.equals("help")) {
        ...
        helpMsg
            ...
            .append("time <morning/noon/evening/night/0-24000>`\n");
        helpMsg
            .append("`")
            .append(discordCommandPrefix)
            .append("weather <clear/rain/storm/snow>`\n");
        helpMsg.append("`").append(discordCommandPrefix).append("heal <player>`\n");
        helpMsg
            .append("`")
            .append(discordCommandPrefix)
            .append("gamemode <player> <Creative/Adventure/Spectator>`\n");
        helpMsg
            .append("`")
            .append(discordCommandPrefix)
            .append("give <player> <item_id> [count]`\n");
      ...
      if (action.equals("time")) {
        if (args.length >= 2) {
          ...
          channel
              .sendMessage(
                  "Usage: " + discordCommandPrefix + "time <morning/noon/evening/night/0-24000>")
              .queue();
        }
        return;
      }
      if (action.equals("weather")) {
        if (args.length >= 2) {
          ...
          channel
              .sendMessage("Usage: " + discordCommandPrefix + "weather <clear/rain/storm/snow>")
              .queue();
        }
        return;
      }
      ...
      switch (action) {
        ...
        case "kick":
          payload.addProperty(
              "reason",
              args.length > 2
                  ? cmd.substring(action.length() + playerName.length() + 2).trim()
                  : "Kicked via Discord");
            ...
            channel
                .sendMessage(
                    "Usage: "
                        ...
                        + "gamemode <player> <Creative/Adventure/Spectator>")
                .queue();
            return;
          }
          break;
        case "give":
          if (args.length > 2) {
            ...
            channel
                .sendMessage("Usage: " + discordCommandPrefix + "give <player> <item_id> [count]")
                .queue();
            return;
          }
          break;
    ...
    private void sendApiResponse(
        net.dv8tion.jda.api.entities.channel.middleman.MessageChannel channel, String response) {
      try {
  ...
  private class DiscordCommandSender implements CommandSender {
    ...
    public void sendMessage(@Nonnull com.hypixel.hytale.server.core.Message message) {
      String text =
          com.hypixel.hytale.server.core.util.MessageUtil.toAnsiString(message).toString();
