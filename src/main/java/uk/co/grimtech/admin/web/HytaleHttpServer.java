...
public class HytaleHttpServer {
  ...
  public void start() throws IOException {
    ...
    InetSocketAddress address =
        AdminWebDashPlugin.isReverseProxy()
    ...
    if (AdminWebDashPlugin.useHttps()) {
      try {
        ...
        AdminWebDashPlugin.getCustomLogger()
            .info("[HTTP] Starting HTTPS Server on port " + actualPort);
      } catch (Exception e) {
        AdminWebDashPlugin.getCustomLogger()
            .severe(
                "[HTTP] Failed to create HTTPS server, falling back to HTTP: " + e.getMessage());
      ...
      AdminWebDashPlugin.getCustomLogger()
          .info(
              "[HTTP] Starting HTTP Server on port "
                  + actualPort
                  + (AdminWebDashPlugin.isReverseProxy()
                      ? " (Reverse Proxy Mode - Bound to 127.0.0.1)"
                      : ""));
    }
  ...
  private SSLContext createSSLContext() throws Exception {
    ...
    if (!keystoreFile.exists()) {
      ...
      if (AdminWebDashPlugin.isLetsEncrypt()
          && AdminWebDashPlugin.getDomain() != null
          && !AdminWebDashPlugin.getDomain().isEmpty()) {
        AdminWebDashPlugin.getCustomLogger()
            .info(
                "[HTTP] Attempting Let's Encrypt Certificate Generation for "
                    + AdminWebDashPlugin.getDomain());
        certGenerated =
            uk.co.grimtech.admin.util.LetsEncryptManager.checkAndRenewCertificate(
                AdminWebDashPlugin.getDomain(),
                AdminWebDashPlugin.getLetsEncryptEmail(),
                keystoreFile,
                keystorePassword);
      }
      if (!certGenerated) {
  ...
  private void generateSelfSignedCert(File keystoreFile, String password) throws Exception {
    ...
    ProcessBuilder pb =
        new ProcessBuilder(
            ...
            password,
            "-dname",
            "CN=" + domain + ", OU=Hytale, O=Grimtech, L=City, ST=State, C=US");
  ...
  static class StaticHandler implements HttpHandler {
    ...
    public void preLoadAssets() {
      ...
      AdminWebDashPlugin.getCustomLogger()
          .info("[HTTP] Pre-loaded " + assetCache.size() + " static assets.");
    }
    ...
    public void handle(HttpExchange t) throws IOException {
      ...
      t.getResponseHeaders()
          .set("ETag", "\"" + lastPreloadTime + "_" + resourcePath.hashCode() + "\"");
    ...
    public void handle(HttpExchange t) throws IOException {
      ...
      if (path.startsWith("/.well-known/acme-challenge/")) {
        ...
        String challengeContent =
            uk.co.grimtech.admin.util.LetsEncryptManager.getChallengeContent(token);
      ...
      boolean isPublicEndpoint =
          (path.startsWith("/api/item/") || path.startsWith("/api/mod/")) && path.endsWith("/icon")
      ...
      if (!isPublicEndpoint) {
        ...
        boolean isValidSession =
            uk.co.grimtech.admin.util.AuthManager.isValidSession(sessionId, clientIp);
        boolean isValidToken = expectedToken != null && expectedToken.equals(authToken);

        if (!isValidSession && !isValidToken) {
          AdminWebDashPlugin.getCustomLogger()
              .warning("[HTTP] 401 Unauthorized for path: " + path + " from IP: " + clientIp);
