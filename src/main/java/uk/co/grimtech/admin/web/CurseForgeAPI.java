...
public class CurseForgeAPI {
  private static final String API_KEY =
      "$2a$10$VJ6wYwy6mFhRg3EB7NabEuD/vGWm4QStItRBOBw4uy5tTaSUMnCF2";
        ...
        () -> {
          try {
            ...
            if (!searchQuery.equals(modName)) {
              ...
              if (result != null) {
                getLogger()
                    .info("[CurseForge] Found match for " + modName + ": " + result.toString());
                modCache.put(cacheKey, new CachedMod(result));
                return result;
              }
            }
            ...
            for (String word : wordsToRemove) {
              String simplifiedQuery =
                  searchQuery.replaceAll("\\b" + word + "\\b", "").replaceAll("\\s+", " ").trim();
      ...
      if (response.statusCode() == 200) {
        ...
        if (data != null && data.size() > 0) {
          ...
          for (int i = 0; i < data.size(); i++) {
            ...
            if (cfSlug.equalsIgnoreCase(originalModName)
                || normalizedCfSlug.equals(normalizedModName)) {
              score = 1000;
            } else if (cfName.equalsIgnoreCase(originalModName)
                || normalizedCfName.equals(normalizedModName)) {
              score = 500;
            } else if (cfName.equalsIgnoreCase(searchQuery)
                || normalizedCfSlug.equals(normalizedSearchQuery)) {
              score = 400;
            } else if (normalizedCfName.equals(normalizedSearchQuery)) {
              score = 350;
            } else if (normalizedCfSlug.contains(normalizedModName)
                && normalizedModName.length() >= 5) {
            ...
            } else if (normalizedCfSlug.startsWith(normalizedModName)
                || normalizedCfName.startsWith(normalizedModName)) {
              score = 70;
          ...
          if (bestMatch.has("latestFiles") && bestMatch.getAsJsonArray("latestFiles").size() > 0) {
            JsonObject latestFile =
                bestMatch.getAsJsonArray("latestFiles").get(0).getAsJsonObject();
          ...
          getLogger()
              .info(
                  "[CurseForge] Found mod: "
                      + bestMatch.get("name").getAsString()
                      + " (score: "
                      + bestScore
                      + ")");
          return modInfo;
